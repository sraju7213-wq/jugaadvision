import { aiGenerateStructured, aiAnalyzeVisionMulti } from "./aiGatewayClient";
...
Mood: "${mood || 'Dynamic'}"
${imageContext ? `Multi-Reference Visual Context (Image 1 = Layout/Pose, Image 2 = Art Style, Image 3 = Color Palette — fuse ALL refs):\n${imageContext}\nFUSION RULES: subject/pose/composition from Image 1, medium/technique from Image 2, palette/lighting mood from Image 3.` : ''}
...
export const generateCinematicFromImages = async (
    ...
    try {
        ...
        if (valid.length > 0) {
            ...
            const visionRes = await aiAnalyzeVisionMulti(
                {
                    prompt: "Analyze composition/subjects, style/medium, and colors/lighting of each reference image for cinematic prompt context.",
            ...
            if (visionRes.succeeded < visionRes.total) {
                imageContext += `\n(Note: ${visionRes.total - visionRes.succeeded} reference(s) failed analysis; fused from ${visionRes.succeeded}.)`;
            }
        }
    ...
    try {
        ...
        if (valid.length > 0) {
            ...
            const vision = await aiAnalyzeVisionMulti(
                {
                    prompt: "Extract camera, lighting, materials, composition, and color details from each reference photo.",
                    images: valid.map((im, i) => ({ base64: im.base64, mimeType: im.mimeType, role: roleOf(i, im) })),
                },
                (done, total) => onStatus?.(`Analyzing references ${done}/${total}...`),
            );
            ...
            if (vision.succeeded < vision.total) {
                imageContext += `\n(Note: ${vision.total - vision.succeeded} reference(s) failed analysis; fused from ${vision.succeeded}.)`;
            }
        }

        const fullConcept = `${concept}. Multi-reference visual details (Image 1 = Layout/Pose, Image 2 = Art Style, Image 3 = Color Palette — fuse ALL): ${imageContext}`;
...
export const generateBannerFromImages = async (
    ...
    try {
        ...
        if (valid.length > 0) {
            ...
            const vision = await aiAnalyzeVisionMulti(
                {
                    prompt: "Analyze the product, materials, colors, and layout of each banner reference.",
                    images: valid.map((im, i) => ({ base64: im.base64, mimeType: im.mimeType, role: roleOf(i, im) })),
                },
                (done, total) => onStatus?.(`Analyzing references ${done}/${total}...`),
            );
        ...
        const fullConcept = `${concept}. Multi-reference visual details (fuse ALL refs): ${imageContext}`;
