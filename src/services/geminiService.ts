...
const THINKER_INSTRUCTIONS: Record<string, string> = {
  ...
  cinematic:
    "Act as a cinematographer and film director. Rewrite with shot size, lens, camera movement, lighting ratio, color grade, aspect-safe framing, and editorial pacing. Think A24 meets IMAX.",
  fashion:
    "Act as a fashion editorial photographer for Vogue. Emphasize garment drape and fabric physics, model direction and pose, runway-to-studio lighting, beauty retouch notes, and magazine composition with copy space.",
  product:
    "Act as a commercial product photographer. Emphasize studio softbox layout, material micro-texture, condensation/reflection control, hero angle, scale cues, and e-commerce-ready negative space for typography.",
  portrait:
    "Act as a portrait photographer. Emphasize facial storytelling, catchlights, skin-tone fidelity, bokeh structure, background separation, and intimate but respectful direction.",
  architect:
    "Act as an architectural visualizer. Emphasize massing, facade materials, daylight study, interior-exterior flow, human scale figures, and photorealistic archviz camera (16mm tilt-shift, two-point perspective).",
};
...
export const generateCreativeMix = async (
  ...
  moodOrCustom?: string,
  images?: { base64: string; mimeType: string; role?: string }[],
  ...
  if (images && images.length > 0) {
    const valid = images.filter((im) => im && im.base64);
    if (valid.length > 0) {
      const roleOf = (i: number, im: { role?: string }) =>
        im.role || (i === 0 ? 'layout' : i === 1 ? 'style' : 'palette');
      try {
        onStatus?.(`Analyzing ${valid.length} reference image${valid.length > 1 ? 's' : ''}...`);
        const vision = await aiAnalyzeVisionMulti(
          {
            prompt: "Analyze each reference image: subject/pose/composition, art style/medium, and color palette/lighting. Be concise but specific.",
            images: valid.map((im, i) => ({ base64: im.base64, mimeType: im.mimeType, role: roleOf(i, im) })),
          },
          (done, total) => onStatus?.(`Analyzing references ${done}/${total}...`),
        );
        imageContext = vision.result;
        if (vision.succeeded < vision.total) {
          visionNote = `Note: ${vision.total - vision.succeeded} of ${vision.total} reference(s) could not be analyzed; fused from ${vision.succeeded}.`;
        }
      } catch {
        // ignore — fall through to text-only
        visionNote = "Note: reference images could not be analyzed; using text concept only.";
      }
    }
  }
...
${imageContext ? `Multi-Reference Image Details (Image 1 = Layout/Pose, Image 2 = Art Style, Image 3 = Color Palette — fuse ALL provided refs, do not drop any):\n${imageContext}` : ""}
${visionNote}
FUSION RULES: take subject/pose/composition from Image 1, medium/technique/texture from Image 2, palette/lighting mood from Image 3. Blend with the text concept and style above.
