...
const REF_ROLES = ["layout", "style", "palette"] as const;
...
const CreativeMixer: React.FC<CreativeMixerProps> = ({
  ...
  // Mirror of refImages for unmount cleanup (avoids stale closure)
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Revoke all image URLs on unmount (via ref mirror — state closure would be stale)
  ...
  const handleGenerate = async (promptOverride?: string) => {
    // Prevent double clicks + stale responses from rapid re-generates
    ...
    const stillCurrent = () => isMounted.current && requestIdRef.current === requestId;
    const setStatus = (s: string) => { if (stillCurrent()) setStatusMessage(s); };
    ...
    try {
      // Prepare images — compress per slot to keep 3-image payloads under server limits,
      // and tag each with its fusion role so the model fuses ALL refs.
      setStatus("Preparing reference images...");
      const imagePayloads: Array<{ base64: string; mimeType: string; role: string } | null> = await Promise.all(
        refImages.map(async (img, idx) => {
          if (!img) return null;
          const role: string = REF_ROLES[idx] ?? `ref-${idx + 1}`;
          try {
            const compressed = await compressImage(img.file, {
              ...FAST_VISION_PRESET,
              mimeType: img.file.type === 'image/png' ? 'image/png' : 'image/jpeg',
            });
            return {
      ...
      const validImages = imagePayloads.filter((img): img is { base64: string; mimeType: string; role: string } => img !== null);
      const loadedCount = validImages.length;
      const roleSummary = validImages.map((im) => REF_ROLE_LABELS[im.role] || im.role).join(' + ');
      if (loadedCount > 0) setStatus(`Fusing ${loadedCount} reference${loadedCount > 1 ? 's' : ''} (${roleSummary})...`);
      ...
      if (professionalMode) {
        ...
        setStatus(validImages.length > 1 ? `Fusing ${validImages.length} references (${roleSummary})...` : "Creating creative mix...");
      ...
      if (stillCurrent()) {
        ...
        if (errorMessage.includes("413") || errorMessage.includes("exceeds") || errorMessage.includes("Payload")) {
          setError("Reference images are too large combined. Try smaller files (each auto-compresses to ~1.2MB) or fewer images.");
