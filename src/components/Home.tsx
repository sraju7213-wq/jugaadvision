...
const HERO_WORDS = ["every field.", "every frame.", "every medium.", "every vision."];
const HERO_STATS: Array<{ value: number; suffix: string; label: string }> = [
  { value: 5, suffix: "", label: "Core Studio Tools" },
  { value: 10, suffix: "", label: "Creative Disciplines" },
  { value: 7, suffix: "", label: "Directorial Layers" },
  { value: 100, suffix: "%", label: "Creator Crafted" },
];
const TICKER_ITEMS = ["Prompt Builder", "Image-to-Prompt", "Creative Mixer", "Batch Matrix", "Pro Prompter", "Banner Studio", "Photography", "Cinematic", "Fashion", "Architecture", "Motion", "3D Latents"];
const Home: React.FC = () => {
  ...
  const [heroWordIdx, setHeroWordIdx] = useState<number>(0);
  ...
  useEffect(() => {
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      setStatValues(HERO_STATS.map((s) => s.value));
      return;
    }
    let raf = 0;
    ...
    const tick = (now: number) => {
      ...
      setStatValues(HERO_STATS.map((s) => Math.round(s.value * eased)));
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => setHeroWordIdx((i) => (i + 1) % HERO_WORDS.length), 2800);
    return () => window.clearInterval(id);
  }, []);

  useEffect(() => {
    const els = Array.from(document.querySelectorAll(".home-studio .reveal"));
  ...
  const handlePanelPointer = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    panel.style.setProperty("--tilt-y", `${(px * 7).toFixed(2)}deg`);
    panel.style.setProperty("--tilt-x", `${(-py * 7).toFixed(2)}deg`);
  };
    ...
    <div className="home-studio">
      <div className="home-studio__inner">
        {/* =========================================================================
            ...
            ========================================================================= */}
        <section ref={heroRef} onPointerMove={handleHeroPointer} className={`studio-hero hero-enter${heroMounted ? " hero-enter--in" : ""}`} aria-labelledby="studio-hero-title">
          <div className="hero-glow" aria-hidden="true" />
          <div className="studio-hero__copy">
            <p className="studio-eyebrow hero-enter__item" style={{ "--enter-delay": "0ms" } as React.CSSProperties}>
              <span className="studio-eyebrow__mark" aria-hidden="true" />
              Jugaad AI <span>/</span> Creative Director Intelligence
            </p>
            <h1 id="studio-hero-title" className="studio-display hero-enter__item" style={{ "--enter-delay": "90ms" } as React.CSSProperties}>
              Master prompt
              <br />
              generation <em>across</em>
              <br />
              <span key={HERO_WORDS[heroWordIdx]} className="hero-word">{HERO_WORDS[heroWordIdx]}</span>
            </h1>
            <p className="studio-hero__lede hero-enter__item" style={{ "--enter-delay": "180ms" } as React.CSSProperties}>
            ...
            <div className="studio-actions hero-enter__item" style={{ "--enter-delay": "260ms" } as React.CSSProperties}>
              <button
                type="button"
            ...
            <div className="studio-hero__meta hero-enter__item" style={{ "--enter-delay": "340ms" } as React.CSSProperties} aria-label="System highlights">
              {HERO_STATS.map((stat, i) => (
                <div key={stat.label}>
                  <strong>{String(statValues[i] ?? 0).padStart(stat.value >= 100 ? 3 : 2, "0")}{stat.suffix}</strong>
          ...
          <div ref={panelRef} onPointerMove={handlePanelPointer} onPointerLeave={resetPanelTilt} className="prompt-panel tilt hero-enter__item" style={{ "--enter-delay": "200ms" } as React.CSSProperties} aria-label="Prompt intelligence preview">
        ...
        <div className="ticker reveal" aria-label="Creative disciplines ticker">
          <div className="ticker__track">
            {TICKER_ITEMS.map((item) => (
              <span key={`a-${item}`} className="ticker__item">{item}<i aria-hidden="true">✦</i></span>
            ))}
            {TICKER_ITEMS.map((item) => (
              <span key={`b-${item}`} className="ticker__item" aria-hidden="true">{item}<i aria-hidden="true">✦</i></span>
            ))}
          </div>
        </div>
        ...
        {/* =========================================================================
            ...
            ========================================================================= */}
        <section className="studio-capabilities studio-section reveal !mt-12" aria-labelledby="capabilities-title">
          ...
          <div className="mt-8 p-5 sm:p-7 border border-[var(--studio-rule)] bg-[var(--studio-surface)] backdrop-blur-md shadow-sm">
            ...
            <div key={sandboxIdx} className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-5 items-stretch swap-fade">
        ...
        {/* =========================================================================
            ...
            ========================================================================= */}
        <section className="studio-spectrum studio-section reveal !mt-16" aria-labelledby="spectrum-title">
        ...
        <section
          ...
          className="studio-section reveal pt-10 border-t-2 border-[var(--studio-ink)]"
        ...
        >
          ...
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div key={activeChapter} className="lg:col-span-8 p-6 sm:p-9 border border-[var(--studio-rule)] bg-[var(--studio-surface-strong)] relative overflow-hidden swap-fade">
          ...
          <div className="mt-12 p-6 sm:p-8 border border-[var(--studio-rule)] bg-[var(--studio-surface)] reveal">
            ...
            <div key={activeComparisonIdx} className="grid grid-cols-1 md:grid-cols-2 gap-6 swap-fade">
              {/* Bad / Raw Prompt Card */}
        ...
        {/* =========================================================================
            ...
            ========================================================================= */}
        <section className="studio-intelligence studio-section reveal" aria-labelledby="intelligence-title">
        ...
        {/* =========================================================================
            ...
            ========================================================================= */}
        <section className="studio-endcap reveal" aria-label="Continue your creative direction">
          <div>
