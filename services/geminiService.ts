...
import { createAIClient, executeWithRetry, reportKeyError, reportKeySuccess, PRIMARY_MODEL, STRUCTURED_MODEL, LOW_COST_MODEL } from "./apiKeyManager";
...
export const rewritePrompt = async (
  ...
  return executeWithRetry('structured_json', async (ai) => {
...
export const describeImageToText = async (
  ...
  return executeWithRetry('text_generation', async (ai) => {
    ...
    try {
      const response: GenerateContentResponse = await ai.models.generateContent({
        ...
        contents: `You are an expert project planner and AI assistant. Given the user's goal, generate a structured Chain-of-Thought (CoT) skeleton.
              ...
              6. Output JSON ONLY. No markdown formatting.`,
        config: {
          responseMimeType: "application/json",
        }
      });
